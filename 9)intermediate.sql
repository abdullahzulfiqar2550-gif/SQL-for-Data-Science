...
#FullText
ALTER TABLE employees ADD fulltext(remarks);
SELECT * from employees WHERE MATCH(remarks) AGAINST("Excellent employee");
SELECT * from employees where match (remarks) against('*employee' IN BOOLEAN MODE);
...
start transaction;
update employees SET salary=salary+500 where emp_id=3;
savepoint sp1;
update employees set salary=salary+1000 where emp_id=4;
rollback to sp1;
commit;
